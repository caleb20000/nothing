...
namespace SM4
{
    class SM4Utils
    {
        ...
        {
            if ((ctx.isPadding) && (ctx.mode == SM4_calss.SM4_ENCRYPT))
            {
                input = padding(input, SM4_calss.SM4_ENCRYPT);
            }
            ...
            if (ctx.isPadding && ctx.mode == SM4_calss.SM4_DECRYPT)
            {
                bous = padding(bous, SM4_calss.SM4_DECRYPT);
            }
            return bous;
        }
        ...
        private long GET_ULONG_BE(byte[] b, int i)
        {
            ...
            return ArrayConversion(n);
        }
        public long ArrayConversion(long k)
        {
            long kr = 0;
            if (k > 2147483648)
            {
        ...
        public int ArrayConversionInt(byte e)
        {
            int aa = e;
            if (e > 128)
            {
...
0x10171e25,0x2c333a41,0x484f565d,0x646b7279 }; 
        ...
        {
            return ArrayConversion((x & 0xFFFFFFFF) << n);
        }

        private long ROTL(long x, int n)
        {
            return ArrayConversion(SHL(x, n) | x >> (32 - n));
        }
        ...
        public string Decrypt_ECB(byte[] cipherText,byte[]skey)
        {
            SM4_Context ctx = new SM4_Context();
            ctx.isPadding = true;
            ctx.mode = SM4_calss.SM4_DECRYPT;
            SM4_calss sm4 = new SM4_calss();
            sm4.sm4_setkey_dec(ctx, skey);
            ...
            {
                string hex = BitConverter.ToString(decrypted).Replace("-", string.Empty);
                return hex;
            }
        }
